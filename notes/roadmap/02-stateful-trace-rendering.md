...
> obligation-spans** faces are the **Arc 2** testing surface and now live under
...
|---|---|---|---|
| **obligation system** | rules declare promises; an undischarged bound = definite failure; spans + four-valued verdict are its faces | — (zizek-side; strong search → tier 3) | ready · **next** |
| **ledger engine (braid)** | the *concurrency* renderer — one lane per thread; activates the cross-thread link connectors | gated: concurrency IR | future |
...
obligation work instead:

1. **obligation-api (bounded)** — highest near-term value, no engine change,
   renderer-independent; its verdict/spans faces are cheap once it exists, and
   spans render in the existing ledger's annotation region (a detail-region
   setting — no braid needed; see `notes/decisions/report-visual-grammar.md`).
2. **the braid** *(future — gated on the concurrency IR)* — this is the
   *concurrency* renderer (one lane per thread); it activates the cross-*thread*
   link connectors the shipped `linkMode = Auto` currently parks (sequential,
   single-thread failures render numeric citations). It is **not** unlocked by
   multi-value sequential traces — several live values are strands on the single
   spine (identity by colour + name), not lanes (see
...
1. **Columns are allocated, never woven.** Each column is owned for its
   occupant's life; freed columns are *reused*, not closed up by shifting
   neighbours. No diagonal column swaps, ever (`╱╲╳` misalign in many fonts
...
the blamed subject always gets the trunk (leftmost) column, and columns
"dead but referenced again later" can be kept while "dead for good" columns are
reclaimed for reuse.
...
> **Re-scoped (lanes mean concurrency).** The braid is the **concurrency**
> renderer — one lane per thread — and is **gated on the concurrency IR**, not
> near-term. Multiple live values in a *sequential* test are **strands** on the
> single spine (identity by colour + name), served by the shipped single-trunk
> ledger, **not** lanes. Read the drawings below as the layout-engine
> illustration with lanes = threads; obligations are **spans**, not lanes (see
> `notes/decisions/report-visual-grammar.md`).
...
**ledger format** — `[gutter] [step №] [call → response]` on line one, a
...
elision in the gutter — and the survey ideas become columns and settings
*inside* it.

**There is one renderer; the design space is its settings**: lane semantics
(threads-only — a lane is a concurrent timeline; values are strands on the
spine, not a lane semantics), detail-line content (`±` deltas / citations /
**obligation-spans** / off), revset, direction, glyph & color tables. This
engine hosts the obligation-spans detail setting and the P(fail) sparkline
column. Two ideas stay outside the grammar: the choice-sequence waterfall (a
`--trace=choices` generator/shrinker diagnostic — a different axis entirely) and
the recorded-session transcript (collapses into the repro footer until the
replay verbs exist).

**Shipped down-payment (`linkMode`, 2026-07-03).** The single-trunk ledger's
mid-line **link connectors** (`●─┬─┬─╮` / `◀─╯`) are now gated to *cross-thread*
citations (`linkMode = Auto`): a single-thread failure — every stateful failure
today — renders the numeric `← cites …` list instead, since the connectors
only carry what a number can't when they point across concurrent timelines. The
connector machinery is intact but dormant, so **concurrent schedule exploration
is what activates it** (rationale in
`notes/decisions/stateful-trace-rendering.md` and
`notes/decisions/report-visual-grammar.md`).
...
Color per column, jj-style: gutter glyphs in identity colors, step numbers dim,
responses by outcome, `±` lines dim, `✗` block red. The "failure lands on a
dead lifeline" thesis — after `◌` the strand goes dotted, so the `✗` visibly
lands on a dead value — is a **value-strand** reading, and belongs to the
single-spine ledger. In the concurrency braid a lane is a *thread* (which does
not die); value death rides the strand's glyph/color, not the lane (see
`notes/decisions/report-visual-grammar.md`).

Stress-tested at 14 steps / 5 lanes / 2 reclaims: renders correctly, but
eager compaction made columns drift (one lane occupied 3 columns over its
life) and cost one link row per reclaimed column. **Decision: no-shift
allocation** — reuse freed columns for new threads, never shift; width is
bounded by peak concurrent threads either way, columns stay stable for life,
and most link rows disappear. (This allocation rule is axis-agnostic — it is
the layout engine's, and applies whatever the columns mean.)

#### Scaling & demotion policy

The braid's wall is reader working memory, not character width (2L−1 cells):
comfortable to **~4 concurrent threads, ~25 steps, connectors spanning ≤2
...
always land at 1–3 threads and a handful of steps, i.e. braid territory — so
...
```
peak concurrent threads ≤ 4  ∧  steps ≤ ~25  →  full braid          (all())
else                                          →  1-hop neighborhood  (ancestry(✗) ∪ threads sharing a step with it)
...
One shared five-hue **identity palette** carries strand colors today (a value's
identity on the spine) and the braid's lane colors later (a thread's identity) —
same palette, both axes (this is `paletteColor` in `Hegel.Report.Ann`).

```
reserved:  red        → failure only (✗, diff)
           default fg → step text, structure
           dim (SGR 2)→ elided rows, dead segments (harmless if unsupported)
palette:   cyan, magenta, yellow, blue, green   (in that order; red never)
never:     white/black/gray (theme-fragile)
```

Highest-value use: color the value's *name in the text region* with its strand
color (`write h₂` with magenta `h₂`) — binds graph to text with zero geometry
and disambiguates reused columns. Palette order is colorblind-aware (deutan
confusion pair deferred to the fifth hue). Five distinguishable hues ≈ the
4-thread readability wall: the budgets agree.
...
- **concurrent lanes** *(future — the braid, gated on the concurrency IR)*. When
  concurrent stateful testing lands (deterministic, replayable schedule
  exploration — loom/shuttle/dejafu-style, *not* observational linearizability),
  each lane is a **thread** and the counterexample is one concrete interleaving
  (a total order), so it renders through the braid above with lanes = threads,
  plus preemption link-rows and a thread tag. No invoke/return boxes, no
  happens-before partial order, no Elle/Knossos "no linearization exists" cycle —
  those belong to observational testing, which the replay architecture argues
  against. There is one lane semantics now: threads. See
  `notes/decisions/report-visual-grammar.md`.
...
- **obligation-api** *(the core; zizek-side)*. The obligation API lives on
  ...
  *search* strong — the one piece that must not be planned on (`gated: engine`;
  ...
  guess the shape wrong) is introduced together with the spans.
- **four-valued verdict header** *(verdict face)*. definitely / probably
  false; an open obligation span ends in a give-up terminator (`?`) instead of
  `✗`. See Mockup D for what the header buys once obligations exist.
- **obligation-spans** *(rendering face; a detail-region setting of the shipped
  ledger — like the `±` delta rows — not a lane semantics)*. An obligation is a
  **span**: an incur→resolve interval drawn in the right (annotation) region,
  **not** a second gutter lane (lanes are reserved for concurrent threads — see
  `notes/decisions/report-visual-grammar.md`). The four-valued verdict rides the
  span terminator — `✗` violated / `?` open-at-trace-end / `▪` discharged; the
  incur→resolve edge is where the connector vocabulary genuinely pays off. Since
  the span lives in the ledger's annotation region, obligation rendering is **not
  braid-gated** — it ships on the single-spine ledger. (This section previously
  proposed a second gutter column and contradicted Mockup D, which already draws
  the span treatment; the span treatment is now the plan.)
  ...
  The concrete rendering (span glyph stock, multi-open-span layout, interaction
  with the link/call-width budgets) is designed when the obligation API lands —
  the sketch above fixes only the grammar (spans, not lanes).
...
- Rough survey renderings (state-transition chain, citation prose block,
  ...
  obligation-spans, and the repro footer respectively.
...
What the four-valued header buys once obligations exist. The span ends in
`?`, not `✗`; the headline says gave-up, not counterexample. (Note this draws
the obligation as a right-region span — `□──╮ … ◀──╯` — the treatment now
adopted everywhere; see the `obligation-spans` section above.)
