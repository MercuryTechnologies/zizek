...
- **Link connectors gated to cross-lane citations** (`linkMode`, default
  `Auto`). The mid-line connectors only carry what a numeric list can't when a
  citation points to a *different* lifeline. Today `Blame.citationsFor` cites
  only the blamed value's own lineage chain, so every citation is same-lane
  and the connectors merely duplicate the per-row `observed` annotations while
  costing call-column width. `Auto` draws connectors only when a citation is
  genuinely cross-lane (`Trace.root citedVar /= Trace.root subject` — a *root*
  compare, not `Var.pool`, so a `Pool.transfer` that keeps one logical value
  is not mistaken for a lane crossing); single-lane reports render the numeric
  `← cites …` list instead. Connectors return automatically once the
  multi-lane braid produces cross-lane citations. `Links`/`Numeric` force
  either mode (the link-geometry pins run under `Links`).
...
- `just test unit` — `PoolEvents`, `TraceIR`, `LedgerRendering` (byte-exact
  ...
  plus the `Auto` default rendering numeric citations on single-lane traces;
