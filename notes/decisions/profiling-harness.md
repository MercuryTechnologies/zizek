...
At capture time the hot FFI imports were `safe` ccalls, so their time did **not** tick in the `.prof` — comparing `.prof` ticks against `.rts` MUT elapsed gave the split directly. (**Method note**: since fix #6 those imports are `unsafe`, so engine time now ticks inside the calling cost centres — `.prof` ticks no longer isolate the Haskell share; use native sampling under "Deferred" if the split is needed again.) `mixed`: ~3.0s of Haskell ticks vs 9.8s MUT elapsed → roughly 30% Haskell / 70% engine (profiling inflates the Haskell share). GC is a non-issue everywhere (97%+ productivity) despite ~2MB/case allocation on mixed.
...
6. **`safe` ccalls on the per-draw path** — ✅ **fixed 2026-07-02**: each safe call cost ~0.1–0.5µs over `unsafe` (capability release/reacquire), a real fraction of the 1.1µs/draw floor (post-#4, `$wgenerate` + `$wstateMachineNextRule` = ~43% of mixed Haskell ticks). All 14 per-test-case primitives (`hegel_generate`, spans, collections, pools, state machine, `hegel_primitive_boolean`, `hegel_target`, `hegel_mark_complete`) are now imported `unsafe`, **verified against `references/hegel-rust/hegel-c`**: they execute **inline on the calling thread** (`tc.ds` → `NativeDataSource.with_ntc` under an uncontended `Mutex` — the worker is parked while the caller drives the case), never call back into Haskell, never touch disk: `mark_complete` is in-memory (`ntc.conclude` plus a non-blocking MPSC ack; DB persistence happens in the worker's run loop), engine panics are caught worker-side (`catch_unwind`) and surface through `hegel_next_test_case`, and unicode tables are `include_str!`-embedded, parsed once into a `OnceLock` (first text draw pays a one-time bounded CPU parse). Result (hyperfine A/B, same machine, `profiles/{pre,post}-unsafe/wallclock.md`): wall-clock draws −19%, steps −15%, shrink −12%, mixed −10%, baseline −9%; startup-dominated micro-scenarios (`gen-hoard`, `shrink --no-shrink`) within noise.
   - *Still `safe` (must not change)*: `hegel_next_test_case` (blocks on the worker channel; shrink bookkeeping and database IO happen behind it) and `hegel_run_free` (joins the worker thread).
   - *The asterisk, accepted*: `Backend::Urandom` — selected explicitly or **automatically under Antithesis** — opens and reads `/dev/urandom` inside every entropy-consuming call (not just `hegel_generate`: also `collection_more`/`reject`, `pool_generate`, `state_machine_next_rule`, `primitive_boolean`). Under `unsafe` that pins the capability (and delays GC sync) for a syscall pair per draw. Accepted rather than dual `safe`/`unsafe` imports because `/dev/urandom` never blocks by spec, the pin is µs-scale, and Antithesis freezes the whole VM for determinism anyway — dual-import dispatch plumbing through `TestCase`/`DataSource` would buy nothing measurable.
