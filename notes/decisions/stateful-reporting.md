...
run's narrative in chronological order, which Aggregate re-sorts by code position;
...
Note for the trace-rendering work (`notes/roadmap/02-stateful-trace-rendering.md`):
its proposed report composition keeps Timeline's failing-step splice but
demotes it from the whole report to a panel beneath a
