...
- `library/Hegel/Runner.hs` — `check`: drives the `libhegel` engine, applies `Settings`, pumps test cases, replays reproduction blobs. A run the engine declared nondeterministic reports its failure's origin with no reproducer, since such a run carries no reproduce blob and never replays
...
`Hegel.Stateful.Concurrent.run` drives the same round protocol with more than one worker: the root test case still drives `next_group` and, once a round concludes, the invariant checks, while each worker dispatches rules against its own clone, acquired once and held for the whole case. It opens no `LabelStatefulRule` span, since a round's rules draw on different clones and a span opened on the root would not enclose any of them, and every worker's `Env` forces `journal = Silent` regardless of the ambient state, since a worker's clone runs its own `Tick` clock, making cross-worker note order meaningless, and several workers journaling into one buffer would race. A rule's group id is interned to a dense `Int64` by first appearance and handed to `hegel_new_state_machine`'s `rule_groups` array; two rules in the same group may share a round, rules in different groups never do. Creating a machine whose bounds ever allow `max_concurrency > 1` declares the whole run nondeterministic per `libhegel`'s own contract, and the run's first such creation always discards that case; a failure from such a run reports the engine's failure origin and a footnote stating there is no reproducer, not the richer step-by-step trace `Hegel.Stateful.run` gives. See `notes/roadmap/03-concurrent-stateful.md` for what's still deferred (live per-case capture, cross-worker trace folding, and the fuller reproduction reporting that live capture unlocks).
