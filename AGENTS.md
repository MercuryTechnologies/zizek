...
- Design work lives in `notes/`: `notes/decisions/` holds decision records for shipped work (rationale not to be re-litigated); `notes/roadmap/` holds upcoming work in priority order (`01-…` is next up). Read the relevant note before starting work it covers, and keep it current as decisions change — including moving a roadmap note to `decisions/` when its work ships.
