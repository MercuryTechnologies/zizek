...
layoutRows opts trace blame = orient <> footerRows
  where
    ...
    -- (threads). No trace has more than one thread yet, so 'Auto' always
    -- renders the numeric citation list. A citation to another *value* in a
    -- sequential test is not a crossing — values are strands on one spine, not
    -- lanes (see notes/decisions/report-visual-grammar.md); enriching 'Blame'
    -- to cite across values must not re-activate the connectors here.
    crossThreadCitations = False
    linkWanted = case opts.linkMode of
      Links -> True
      Numeric -> False
      Auto -> crossThreadCitations
