...
--      step timeline, ledger, or footer
...
--   4. file handles — the flagship citation ledger: a use-after-close bug
...
--      so it degrades to the step timeline with a compact trajectory lead
--
...
-- (reason-led headline, ledger, the failing step's splice, footer); scenario 5's
-- flat story degrades to the step timeline with a trajectory lead.
--
...
main = do
  ...
  runLedgerScenario True "4: file handles — use-after-close, citation ledger" (Stateful.run fileHandleMachine)
  runLedgerScenario False "5: poked value — flat story, degrades to trajectory" (Stateful.run overflowMachine)
...
-- now composes the verdict headline, citation ledger, the failing step's
...
-- no step timeline, ledger, or footer.
...
-- * Scenario 4: file handles (citation ledger, flagship)
